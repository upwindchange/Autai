...
const GroupedThreadList: FC<{ selectedTagId: number | null }> = ({
  ...
  const { tagGroups, untagged } = useMemo(() => {
    ...
    for (const thread of threads) {
      if (selectedTagId !== null && !thread.tags.some((t) => t.id === selectedTagId)) {
        continue;
      }
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {untagged.length > 0 && (
        <CollapsibleTagGroup
          group={{ tagName: "Untagged", tagColor: "bg-muted text-muted-foreground", threads: untagged }}
          activeThreadId={mainThreadId}
          onSwitch={(threadId) => aui.threads().switchToThread(threadId)}
        />
      )}
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        ...
        />
        <span className={`rounded px-1.5 py-0 text-[10px] font-medium ${group.tagColor}`}>
          {group.tagName}
        </span>
        <span className="text-[10px] font-normal">({group.threads.length})</span>
      </button>
      {open && (
      ...
      <DropdownMenuContent side="bottom" align="start" className="min-w-36">
        <DropdownMenuSub>
          <DropdownMenuSubTrigger
            ...
            className={availableTags.length === 0 ? "opacity-50 pointer-events-none" : ""}
          >
          ...
          <DropdownMenuSubContent>
            {availableTags.map((tag) => (
              ...
              >
                <span className={`inline-flex rounded px-1.5 py-0 text-[10px] font-medium ${getTagColor(tag.id)}`}>
                  {tag.name}
                </span>
              </DropdownMenuItem>
            ))}
