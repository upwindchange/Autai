...
export function DeltaValue({ value, options }: DeltaValueProps) {
	...
	const colorClass =
		isGood ? "text-green-700 dark:text-green-500"
...
export function StatusBadge({ value, options }: StatusBadgeProps) {
	...
	const variant =
		config.tone === "danger" ? "destructive"
		: config.tone === "neutral" ? "outline"
		: "secondary";
...
export function PercentValue({ value, options }: PercentValueProps) {
	...
	const signed =
		numeric > 0 && showSign ? `+${absoluteFormatted}`
		: numeric < 0 ? `-${absoluteFormatted}`
		: absoluteFormatted;
