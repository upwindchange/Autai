...
export const MessageItem = memo(
  function MessageItem({ message }: MessageItemProps) {
    ...
    // Only use llm-ui processing for assistant messages
    // and memoize the block configuration
    const llmConfig = useMemo(
      ...
      [message.content, message.isComplete]
    );

    const { blockMatches } = useLLMOutput(llmConfig);
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <div className={cn("px-4 py-3", isUser ? "prose-invert" : "")}>
              {isUser ? (
  ...
  (prevProps, nextProps) => {
    ...
    // Only re-render if content, sender, or completion status changes
    return (
