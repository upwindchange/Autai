...
import type { Task, Page, View, Agent, AppState, StateChangeEvent } from '../../electron/shared/types';
...
function restoreTaskPages(task: Task): Task {
  return {
    ...task,
    pages: task.pages instanceof Map ? task.pages : objectToMap(task.pages as any)
  };
}
  ...
  subscribeWithSelector((set, get) => ({
    ...
    createTask: async (title?: string, initialUrl?: string) => {
      try {
        ...
        const defaultUrls = ['https://www.reddit.com', 'https://www.amazon.com'];
        const defaultUrl = initialUrl || defaultUrls[Math.floor(Math.random() * defaultUrls.length)];
    ...
    deleteTask: async (taskId: string) => {
      try {
        const result = await window.ipcRenderer.invoke('app:deleteTask', { taskId });
        if (!result.success) {
    ...
    addPage: async (taskId: string, url: string) => {
      try {
        const result = await window.ipcRenderer.invoke('app:addPage', { taskId, url });
        if (!result.success) {
    ...
    deletePage: async (taskId: string, pageId: string) => {
      try {
        const result = await window.ipcRenderer.invoke('app:deletePage', { taskId, pageId });
        if (!result.success) {
    ...
    selectPage: async (taskId: string, pageId: string) => {
      try {
        const result = await window.ipcRenderer.invoke('app:selectPage', { taskId, pageId });
        if (!result.success) {
        ...
        const state = get();
        if (state.activeViewId && state.containerBounds && !state.isViewHidden) {
    ...
    navigate: async (pageId: string, url: string) => {
      try {
        const result = await window.ipcRenderer.invoke('app:navigate', { pageId, url });
        if (!result.success) {
    ...
    handleStateChange: (event: StateChangeEvent) => {
      ...
      switch (event.type) {
        ...
        case 'ACTIVE_VIEW_CHANGED':
          ...
          if (event.viewId && currentState.containerBounds && !currentState.isViewHidden) {
            window.ipcRenderer.invoke('app:setViewBounds', {
              viewId: event.viewId,
              bounds: {
