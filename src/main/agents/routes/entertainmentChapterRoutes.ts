...
import { dehydrateScheduler } from "@agents/workers/entertainmentWorker/dehydrate/scheduler";
import {
  decodeNovelFile,
  ingestNovel,
} from "@agents/workers/entertainmentWorker/dehydrate/ingest";
...
entertainmentChapterRoutes.post("/threads/:threadId/setup", async (c) => {
  try {
...
entertainmentChapterRoutes.post("/threads/:threadId/upload", async (c) => {
  try {
    const threadId = c.req.param("threadId");
    const body = await c.req.json().catch(() => ({}));
...
entertainmentChapterRoutes.get("/threads/:threadId/chapters/:n", (c) => {
  try {
    const threadId = c.req.param("threadId");
...
entertainmentChapterRoutes.get("/threads/:threadId/position", (c) => {
  const threadId = c.req.param("threadId");
...
entertainmentChapterRoutes.post("/threads/:threadId/position", async (c) => {
  try {
    const threadId = c.req.param("threadId");
    const body = await c.req.json().catch(() => ({}));
...
entertainmentChapterRoutes.get("/threads/:threadId/worker", (c) => {
  const threadId = c.req.param("threadId");
...
entertainmentChapterRoutes.post("/threads/:threadId/worker", async (c) => {
  try {
    const threadId = c.req.param("threadId");
    const body = await c.req.json().catch(() => ({}));
