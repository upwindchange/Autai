...
import type { FileNovel } from "@shared";
...
// --- file-novel ingestion (decode → parse → source_chapters) ---------------

/**
 * Read a novel file's raw bytes and decode them to a string, auto-detecting the
 * encoding (jschardet) and converting via iconv-lite.
 *
 ...
 *
 * This is the file route's source-acquisition entry: its contract is the
 * wizard's `FileNovel` (origin metadata, logged) plus the already-decoded text;
 * the parsed chapters land in `source_chapters` ready for the scheduler to
 * rewrite.
 */
export function ingestFileNovel(
  threadId: string,
  novel: FileNovel,
  text: string,
): number {
  ...
  logger.info("ingested file novel", {
    threadId,
