...
Autai supports every major AI provider — OpenAI, Anthropic, Google, DeepSeek, Mistral, xAI, and 100+ more. Use any model from any provider, or even run models locally with Ollama. Bring your own API key, pick your favorite model, and go. See [full list](https://models.dev/).
...
Autai automatically tags your conversations with color-coded labels so you can find them later. Browse your history in a clean list, or switch to a tag-grouped view to see everything by category. Search by keyword, filter by tag, and bulk-archive old threads. The AI even names each conversation for you, so your sidebar stays tidy without any effort.

### You Stay in Control

When the AI hits something it shouldn't handle on its own — a CAPTCHA, a login screen, or a payment form — it pauses and hands it back to you. You approve the plan before it executes complex tasks. You're always in the loop.

### Watch It Work in Real Time

Split-view mode puts the chat on one side and the browser on the other. You can watch every click, every scroll, every form field as the AI fills it in. No black box — you see exactly what's happening.
...
- **Image and file attachments** — Upload a screenshot, a document, or any file, and the AI will incorporate it into the conversation.
...
That's it. No setup wizards, no configuration files, no learning curve.
