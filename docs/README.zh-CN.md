...
**给小说脱水的 AI 编辑，底下是一套全功能的 AI 对话应用。**

娱乐模式是招牌：网文、生肉翻译、本地 TXT 拖进来，它把水拧干、把文笔修好、把语言翻对，再装进一个正经阅读器端给你。而这背后，Autai 本身就是一个不输任何同类的 AI 对话客户端——200+ 家服务商、7000+ 个模型，自建 / 本地 API 也支持，公式、代码、图表直接在对话里渲染，支持 MCP 工具，内置浏览器替你操作，还有 Perplexity 式的研究模式：自己搜、自己读、答案带来源。Key 是你自己的，模型随你挑，全程在你自己的电脑上跑。

[下载](https://github.com/upwindchange/autai/releases) · [娱乐模式](#-娱乐模式网文脱水器) · [功能一览](#功能一览) · [三步上手](#三步上手)
...
- **什么语言的书都能看**——整本翻译成中文、给省略主语的日轻补上「谁在说话」，都不在话下。嫌选项不够？一句话自定义：「文言文翻成白话文，定场诗判词原样保留」——照办。
...
OpenAI、Anthropic、Google、DeepSeek、Mistral、xAI……200+ 家服务商、7000+ 个模型随便切，自建 / 本地 API 也支持。填上 Key 就能用。[完整目录](https://models.dev/)。
...
<table>
  <tr>
    ...
    <td width="50%" valign="top">
      <img src="../docs/screenshots/cofig-models.png" alt="七千多个模型里挑" width="100%" />
      <br /><sub><b>七千多个模型</b>——能搜能筛，一键换。</sub>
    </td>
  </tr>
</table>
...
代码带高亮，公式排版像教材，Mermaid 图——从流程图到数据图表——直接在对话里成图，富文本一样不缺。复杂答案回来看着不头疼。
...
- **开浏览器替你跑腿**——列好步骤、真开浏览器执行：表单、订票、购物车、跨店比价。验证码、登录、支付一到，立刻停车把方向盘还给你。
- **替你查资料**——快速、标准、深度三档。深度调研直接把问题拆开、逐个子话题搜过去读过去，最后连出处一起写给你——相当于对话应用里内置了一个 Perplexity。
...
- **MCP 服务器**——设置里接入外部工具。
- **远程访问**——设个密码把 Autai 挂到局域网，手机浏览器也能用。加 `--remote` 参数启动可临时强制开启，不影响已保存的设置。
- **语音朗读**——回答可以直接读给你听。
- **深色 / 浅色 / 跟随系统**。
- **中文 / English 界面**随时切。
...
2. **填个 API Key**——哪家服务商都行，自建 / 本地 API 也可以。
...
**如果 Autai 让一本注水的书重新值得追，点个 Star ⭐——让更多人少翻半页水文。**
